...
class Atframework_Employees_Carousel extends WP_Widget {

	public function __construct() {
		$widget_ops = array('classname' => 'atframework_employees_carousel_carousel_widget', 'description' => __( 'Show your employees in a carousel', 'astrid') );
        parent::__construct(false, $name = __('Astrid Child: Employees Carousel', 'astrid'), $widget_ops);
		$this->alt_option_name = 'atframework_employees_carousel_carousel_widget';
	...
	function update($new_instance, $old_instance) {
		...
		if ( isset($alloptions['atframework_employees_carousel']) )
			delete_option('atframework_employees_carousel');		  
	...
	function widget($args, $instance) {
		$cache = array();
		if ( ! $this->is_preview() ) {
			$cache = wp_cache_get( 'atframework_employees_carousel', 'widget' );
		}
...
?>
			<?php if ( $title ) echo $before_title . $title . $after_title; ?>
				...
				<div class="employees-area ap-carousel clearfix">
	...
	<?php
		...
		if ( ! $this->is_preview() ) {
			...
			wp_cache_set( 'atframework_employees_carousel', $cache, 'widget' );
		} else {
			ob_end_flush();
		}
	}
